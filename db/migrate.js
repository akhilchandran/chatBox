...
const { getDB, waitDBConnect } = require('../APIServer/db/pg-client');
...
const createUserSql = 'CREATE USER $1:name WITH ENCRYPTED PASSWORD $2';
...
const grantUserTableAccessSql =
  'GRANT SELECT, INSERT, UPDATE, REFERENCES, ' + 'TRIGGER ON ALL TABLES IN SCHEMA public TO $1:name;';
...
async function runMigrations(config, logger) {
  try {
    ...
    if (!pgConfig.user || !pgConfig.password) {
      const errorMessage = 'Config pgConnection missing user and password.';
      logger.error(errorMessage);
      return Promise.reject(new Error(errorMessage));
    }

    const migrationAdminDB = getDB(config.pgMigrationConnection);
    ...
    for (let i = 0, len = patchesToRun.length; i < len; i++) {
      ...
      try {
        ...
        const errorMessage = `Running ${filename}: ${error.stack}`;
